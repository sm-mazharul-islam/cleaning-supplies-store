...
//   /**
//    * LOGOUT HANDLER
//    * Clears the JWT from local storage and redirects user
//    */
//   const handleLogout = () => {
//     localStorage.removeItem("token"); // Remove the auth credential
...
//               <p className="text-xs text-slate-500">Premium Admin</p>
//             </div>
...
const DashboardSideBar: React.FC<DashboardProps> = ({ children }) => {
  ...
  // 1. Define nav items with allowed roles
  const navItems = [
    { name: "Home", href: "/", icon: FaHome, roles: ["admin", "user"] },
    {
      name: "Profile",
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] font-sans text-slate-900">
      ...
      {/* SIDEBAR ASIDE */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900 text-white transform transition-all duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static lg:inset-0 shadow-2xl`}
      >
        <div className="flex flex-col h-full">
          <div className="p-8 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-bold">Supplies.io</h2>
            </div>
            <button
              className="lg:hidden p-2 hover:bg-slate-800 rounded-lg"
              onClick={toggleSidebar}
          ...
          <div className="p-6 mt-auto border-t border-slate-800">
            <button
              onClick={handleLogout}
              className="flex w-full items-center gap-4 py-3 px-4 rounded-xl text-slate-400 hover:bg-red-500/10 hover:text-red-500 transition-all"
            >
              <FaSignOutAlt />
              <span className="font-medium">Logout</span>
            </button>
